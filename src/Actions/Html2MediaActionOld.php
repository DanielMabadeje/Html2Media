...
class Html2MediaActionOld extends Action
{
    ...
    public function savePdfAction(): Action
    {
        return Action::make('SavePdfs')
            ...
            ->action(
                fn(Action $action) =>
                $action->getLivewire()->dispatch('triggerPrint', ...$this->getDispatchOptions('savePdf'))
            );
    }
    ...
    public function printAction(): Action
    {
        return Action::make('Print')
            ...
            ->action(
                fn(Action $action) =>
                $action->getLivewire()->dispatch('triggerPrint', ...$this->getDispatchOptions('print'))
            );
    }
}
